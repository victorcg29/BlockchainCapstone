...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address toAddress, uint256 tokenId) public {
        ...
        require(toAddress != owner,"Given address is the owner of the token" );

        // require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == owner || isApprovedForAll(owner, msg.sender),"Not allowed to approve");

        // add 'to' address to token approvals
        _tokenApprovals[tokenId] = toAddress;

        // emit Approval Event
        emit Approval(msg.sender, toAddress, tokenId);
    }
    ...
    // TIP: remember the functions to use for Counters. you can refresh yourself with the link above
    function _transferFrom(address fromAddress, address toAddress, uint256 tokenId) internal {

        // require from address is the owner of the given token
        require(fromAddress == ownerOf(tokenId),"Not the owner of the token");
        // require token is being transfered to valid address
        require(toAddress != address(0),"require token is being transfered to valid address");
        // clear approval
        _clearApproval(tokenId);
        // update token counts & transfer ownership of the token ID
        _ownedTokensCount[fromAddress].decrement();
        _ownedTokensCount[toAddress].increment();
        _tokenOwner[tokenId]= toAddress;
...
contract VCGToken is ERC721Metadata("VCGToken","CGV","https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/"){

    function mint(address toAddress, uint tokenId) public onlyOwner returns(bool){
